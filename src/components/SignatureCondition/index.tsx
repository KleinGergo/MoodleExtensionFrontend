...
const handleFieldChange = (fieldName: string, value: any) => {
  ...
  const generateJSON = () =>{
   ...
   console.log(JSON.stringify(signatureConditionWrapper, (key, value) => {
      if (value !== 0) return value
    }))
    if (subjectIdentifier !== '')
    {
      API.addSignatureCondition(subjectIdentifier,JSON.stringify(signatureConditionWrapper, (key, value)=>{
        if (value!==0) return value
      }));
    ...
    <>
      ...
      <h1 className='cards__header__text'>Aláírás feltétel létrehozása</h1>
        ...
        <div className='cards__container'>
         <div className='cards__item__link'>
          ...
          >
            ...
            <option value="offeredGrade">Megajánlott Jegykialakítás</option>
          </select>
        </div>
        ...
        {selectedType === 'offeredGrade' && (
          <div id="offeredGradingContainer">
            <h4 className='headers'>Jeles eredmény alsó határa (százalékban): </h4>
            <input
              min="0"
              max="100"
...
/>
                    ...
                    {condition.RequiredIndividualBigTestPercentage && (
                      <>
                        Szükséges átlagos eredmény: {condition.RequiredIndividualBigTestPercentage}<br />
                        {/* Add more specific elements for this type */}
                      </>
                    )}
                    {condition.Weight && (
